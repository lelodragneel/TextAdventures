...
public class Woods {
		...
		while(core.getPlayer().getHealth() > 0 && currMon > 0) {
			...
			while(exists == false) {
				...
				for(int i = 0; i < options.length-1; i++) {
					if(attack.equalsIgnoreCase(options[i]))
						exists = true;
				}
				if(exists == false)
			...
			while(exists == false) {
				...
				defend = sc.nextLine();
				for(int i = 0; i < options.length-1; i++) {
					if(defend.equalsIgnoreCase(options[i]))
						exists = true;
				}
				if(exists == false)
			...
			if(compAttack.equalsIgnoreCase(defend))
				System.out.println("The monster missed!");
			else {
				core.getPlayer().setHealth(core.getPlayer().getHealth() - 1);
				System.out.println("The monster has hit you in the " + compAttack + "! You have " + core.getPlayer().getHealth() + "/" + core.getPlayer().getMaxHealth() + ".");
			}
			if(compDefend.equalsIgnoreCase(attack))
				System.out.println("You missed!");
			else {
				currMon-=core.getPlayer().getAttack();
				if(currMon < 0)
					currMon = 0;
				System.out.println("You have hit the monster in the " + attack + "! It has " + currMon + "/" + hpMon[rand] + " left!");
			}
		}
		if(currMon < 1)
			return true;
	...
	public String goBack(String p) {
		...
		if (p.equalsIgnoreCase("") || p.length() == 1)
			return backOne;
		else {
			...
			//Choosing a path.
				...
				if(pathFinder(path) == 0) {
					...
					if(temp.equalsIgnoreCase("y")) {
						...
						while (chances != 0){
							...
							//Correct answer.
							if(temp.equalsIgnoreCase("lrlllrl")) {
				...
				//The monster goal of the forest to leave the forest.
					...
					if(spellbook == true) {
						...
						System.out.println("You arrive upon the final monster, but without the ability to hold off his magic, he sends you back to the beginning of the woods.");
						path = "";
						boss = false;
					}
				}
